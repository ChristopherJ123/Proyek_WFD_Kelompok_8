...
@section('content')
    <div class="flex gap-4 w-full m-4">
        <div class="flex flex-col w-full gap-4 overflow-y-auto">
            ...
            <hr class="border-1">
            <form action="{{ route('topics.posts.update', [$topic, $post]) }}" method="post" enctype="multipart/form-data" class="flex flex-col bg-brand-300 p-4 rounded-2xl gap-4 font-sans">
                @csrf
                @method('PUT')
                <div class="flex">
                ...
                <div class="relative">
                    <input class="bg-brand-100 focus:outline-brand-500 focus:outline-2 w-full p-4 rounded-3xl text-lg peer placeholder-transparent" type="text" name="title" id="title" placeholder="Title*" value="{{ $post->title }}">
