            ...
            <div class="flex flex-col absolute top-82 w-full px-8">
                ...
                <div class="flex flex-col gap-8">
                    ...
                    @foreach($bestPosts as $post)
                        <div class="flex flex-col gap-2 text-white rounded-4xl bg-brand-300 p-4 last:mb-4">
                            <div class="flex flex-col gap-3">
                                <div class="tracking-normal font-sans tracking-normal mx-8 text-xl font-medium line-clamp-2">
                                    {{ $post['title'] }}
                                </div>
                                @if(count($post->images) > 0)
                                    <img class="max-h-96 mx-8 object-cover rounded-2xl" src="{{ asset('storage'.$post->images[0]->image_link) }}" alt="{{ $post->images[0]->image_link }}">
                                @endif
                                <div class="flex gap-4">
                                    <div class="flex items-center p-2 px-3 gap-3 rounded-4xl bg-brand-900">
                                        <div class="font-sans font-semibold text-xl">{{ $bestPosts[$loop->index]->upvote_count }}</div>
                                        <button>
                                        ...
                                        <div class="font-sans font-semibold text-xl">{{ $bestPosts[$loop->index]->downvote_count }}</div>
                                        <button>
